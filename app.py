...
class RSSChatAgent:
    ...
    def is_news_query(self, message: str) -> bool:
        news_keywords = ['news', 'latest', 'recent', 'headlines', 'articles', 
                         'updates', 'breaking', 'today', 'current', 'happening', 'stories']
...
def whisper_transcribe(audio):
    ...
    try:
        ...
        transcription = str(decoder_output[0])
...
def generate_text(prompt):
  ...
  if news_response:
        ...
        generated_text = ''.join(chr(min(max(id, 32), 126)) for id in output[0][0][:100])  # Limit output
...
def run_kokoro(input_text):
    ...
    try:
        ...
        output_text = ''.join(chr(min(max(id, 32), 126)) for id in output[0][0][:50])  # Limit output
        return output_text
    except Exception as e:
